...
def run_train_with_adapter(args, hparams, adapter_name, config):
    ...
    print("Extracting modifiers and conjunctions from the datasets...")

    # Initialize sets to hold unique modifiers and conjunctions
    modifiers_set = set()
    conjunctions_set = set()

    # Combine all sentences from training and development datasets
    dataset_sentences = []
    for tree in train_treebank.trees + dev_treebank.trees:
        # Convert tree leaves to sentence
    ...
    for sentence in dataset_sentences:
        ...
        for token in doc:
            ...
            if token.pos_ in ['ADJ', 'ADV']:
                modifiers_set.add(token.text.lower())
            # Extract conjunctions
            if token.pos_ == 'CCONJ':
                conjunctions_set.add(token.text.lower())

    # Add special tokens
    modifiers_set.add('<no_mod>')
    conjunctions_set.add('<no_conj>')

    # Create mappings from words to indices
    mod_vocab = list(modifiers_set)
    mod_to_idx = {mod: idx for idx, mod in enumerate(mod_vocab)}
    mod_vocab_size = len(mod_vocab)

    conj_vocab = list(conjunctions_set)
    conj_to_idx = {conj: idx for idx, conj in enumerate(conj_vocab)}
    conj_vocab_size = len(conj_vocab)

    print("Modifier vocabulary size:", mod_vocab_size)
    print("Conjunction vocabulary size:", conj_vocab_size)
    ...
    # Set embedding dimensions (you can adjust these values)
    mod_embedding_dim = hparams.d_model  # Or another value
    conj_embedding_dim = hparams.d_model  # Or another value
    ...
    parser = parse_chart_PEFT.ChartParser(
        ...
        conj_vocab_size=conj_vocab_size,
        mod_embedding_dim=mod_embedding_dim,
        conj_embedding_dim=conj_embedding_dim,
        mod_to_idx=mod_to_idx, 
        conj_to_idx=conj_to_idx,
    )
