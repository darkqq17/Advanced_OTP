...
from transformers import AutoConfig, AutoModel, AutoModelForSeq2SeqLM # added for pretrained model
...
from transformers.adapters.lora import LoRAConfig
...
class ChartParser(nn.Module, parse_base.BaseParser):
    def __init__(
        ...
        if hparams.use_chars_lstm:
            ...
            if pretrained_model_path is None: # when using pretrained model go in here
                ...
                # Added for LoRA ----------------------------------------------------------------
                # self.pretrained_model.text_decoder.add_adapter("lora_adapter", config=lora_config)
                # self.pretrained_model.set_active_adapters("lora_adapter")
            ...
            if hparams.use_encoder:
                ...
                self.project_pretrained = LoRALinear(
                    d_pretrained, hparams.d_model // 2, location_key="encoder", config=lora_config
                )
                ...
                # self.project_pretrained = LoRALinear(
                #     d_pretrained, hparams.d_model, location_key="encoder", config=lora_config
                # )
        ...
        print("Using LoRA for label prediction.")
        print("bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb")
        self.f_label = nn.Sequential(
            nn.Linear(hparams.d_model, hparams.d_label_hidden),
            # LoRALinear(hparams.d_model, hparams.d_label_hidden, location_key="encoder", config=lora_config),
            nn.LayerNorm(hparams.d_label_hidden),
            nn.ReLU(),
            nn.Linear(hparams.d_label_hidden, max(label_vocab.values())),
            # LoRALinear(hparams.d_label_hidden, max(label_vocab.values()), location_key="encoder", config=lora_config),
        )
        ...
        if hparams.predict_tags:
            ...
            # Modified to use LoRA:
            print("Using LoRA for tag prediction.")
            ...
            self.f_tag = nn.Sequential(
                LoRALinear(hparams.d_model, hparams.d_tag_hidden, location_key="encoder", config=lora_config),
                nn.LayerNorm(hparams.d_tag_hidden),
                nn.ReLU(),
                LoRALinear(hparams.d_tag_hidden, max(tag_vocab.values()) + 1, location_key="encoder", config=lora_config),
            )
    ...
    def forward(self, batch):
        ...
        if self.char_encoder is not None:
            ...
            # Added for LoRA ----------------------------------------------------------------
            # self.pretrained_model = get_peft_model(self.pretrained_model, peft_config).to(self.device)
            ...
            # Added for LoRA ----------------------------------------------------------------
            # features = pretrained_out.encoder_last_hidden_state.to(self.output_device)
